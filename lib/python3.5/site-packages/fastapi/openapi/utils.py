    ...
    for route in routes:
        if getattr(route, "include_in_schema", None) and isinstance(
            ...
            if route.body_field:
                ...
                body_fields_from_blogs_app.append(route.body_field)
            if route.response_field:
                responses_from_blogs_app.append(route.response_field)
            if route.response_fields:
                responses_from_blogs_app.extend(route.response_fields.values())
            if route.callbacks:
                callback_flat_models |= get_flat_models_from_routes(route.callbacks)
            params = get_flat_params(route.dependant)
            request_fields_from_blogs_app.extend(params)
